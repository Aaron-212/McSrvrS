...
actor JavaServerPinger: ServerPinger {
            ...
            await withCheckedContinuation { continuation in
                ...
                connection.stateUpdateHandler = { state in
                        ...
                        Task {
                            do {
                                ...
                                connectionState.resume(
                                    returning: .success(statusData)
                                )
                            } catch {
                                connection.cancel()
                                connectionState.resume(
                                    returning: .failure(
                                        await Self.pingerError(from: error)
                                    )
                                )
                            }
                        }
                    case .failed(let error):
                        ...
                        connectionState.resume(
                            returning: .failure(
                                ServerPingerError.connectionFailed(error)
                            )
                        )
                    case .cancelled:
                        connection.stateUpdateHandler = nil
        ...
        -> ServerStatus.StatusData
    {
        ...
        let latency = try await sendPingAndMeasureLatency(
            connection: connection
        )
        logger.debug(
            "Ping response received from \(host):\(port) with latency \(latency) ms"
        )

        switch JavaServerStatusResponse.parse(jsonString) {
    ...
    private func sendHandshake(
        ...
        data.append(MinecraftProtocolCodec.packVarInt(0))
        ...
        data.append(MinecraftProtocolCodec.packVarInt(hostData.count))
        ...
        data.append(MinecraftProtocolCodec.packVarInt(1))
    ...
    private func sendStatusRequest(connection: NWConnection) async throws {
        let data = MinecraftProtocolCodec.packVarInt(0)  // Packet ID 0x00 for status request
        try await sendData(connection: connection, data: data)
    }

    // Read status response
    private func readStatusResponse(connection: NWConnection) async throws
        -> String
    {
        let data = try await readPacket(
            connection: connection,
            includesStringLength: true
        )
    ...
    private func sendPingAndMeasureLatency(connection: NWConnection)
        async throws -> Int64
    {
        let elapsedTime = try await ContinuousClock().measure {
            let startTime = Int(Date().timeIntervalSince1970 * 1000)  // Current time in milliseconds

            var pingData = Data()
            pingData.append(MinecraftProtocolCodec.packVarInt(1))  // Packet ID 0x01 for ping
            ...
            _ = try await readPacket(
                connection: connection,
                includesStringLength: false
            )
        }

        return await elapsedTime.milliseconds
    }

    // Send data with length prefix
    private func sendData(connection: NWConnection, data: Data) async throws {
        var packet = Data()
        packet.append(MinecraftProtocolCodec.packVarInt(data.count))  // Length prefix
        packet.append(data)
        ...
        try await withCheckedThrowingContinuation {
            ...
            connection.send(
                ...
                completion: .contentProcessed { error in
                    if let error = error {
                        continuation.resume(
                            throwing: ServerPingerError.connectionFailed(error)
                        )
                    } else {
                        continuation.resume()
                    }
    ...
    private func readPacket(
        ...
        var remainingLength = packetLength - MinecraftProtocolCodec.varIntSize(packetID)
        var resultData = Data()
        ...
        if includesStringLength {
            ...
            remainingLength -= MinecraftProtocolCodec.varIntSize(jsonLength)

            // Read the JSON string
            resultData = try await readBytes(
                connection: connection,
                count: jsonLength
            )
        } else {
    ...
    private func readBytes(connection: NWConnection, count: Int) async throws
        -> Data
    {
        return try await withCheckedThrowingContinuation { continuation in
            connection.receive(
                ...
                if let error = error {
                    ...
                    continuation.resume(
                        throwing: ServerPingerError.dataError(
...
nonisolated enum MinecraftProtocolCodec {
    // Pack varint (variable-length integer)
    static func packVarInt(_ value: Int) -> Data {
        var data = Data()
        var val = value
...
nonisolated struct JavaStatusPlayerResponse: Codable {
    let name: String
    let playerId: String
...
nonisolated struct JavaStatusPlayersResponse: Codable {
    let max: UInt32
    let online: UInt32
...
nonisolated struct JavaServerStatusResponse: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        version = try container.decode(
            ServerStatus.Version.self,
            forKey: .version
        )
        players = try container.decodeIfPresent(
            JavaStatusPlayersResponse.self,
            forKey: .players
        )
        favicon = try container.decodeIfPresent(
            String.self,
            forKey: .favicon
        )

        if let motdString = try? container.decode(
            String.self,
            forKey: .motd
        ) {
            motd = motdString
        } else if let motdObject = try? container.decode(
            [String: String].self,
            forKey: .motd
        ),
    ...
    static func parse(_ jsonString: String) -> Result<Self, Error> {
        guard let data = jsonString.data(using: .utf8) else {
            return .failure(
                NSError(domain: "InvalidString", code: 1, userInfo: nil)
            )
        }
...
private nonisolated final class PingConnectionState: @unchecked Sendable {
    ...
    private var continuation:
        CheckedContinuation<JavaServerPinger.PingResult, Never>?
    private var pendingResult: JavaServerPinger.PingResult?
    private var didResume = false

    func setContinuation(
        _ continuation: CheckedContinuation<JavaServerPinger.PingResult, Never>
    ) {
        lock.lock()
...
extension Duration {
    var milliseconds: Int64 {
        let c = components
        return c.seconds * 1_000
            + Int64(c.attoseconds / 1_000_000_000_000_000)
    }
}
